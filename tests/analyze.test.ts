...
test('analyze on the sample data reproduces the $192.50 worklist', () => {
  const r = analyze(s837, s835, csv)
  assert.equal(r.foundMoney.totals.recoverableCents, 19250)
  assert.equal(r.foundMoney.meta.source, 'upload')
})

test('analyze derives the claim lifecycle from the actual 835', () => {
  const r = analyze(s837, s835, csv)
  assert.equal(r.claimTotals.total, 3)
