...
export default function LoginPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(900px 460px at 50% -160px, #eaf0ff 0%, rgba(234,240,255,0) 60%), #f7f5f0', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 396 }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, justifyContent: 'center', textDecoration: 'none', marginBottom: 22 }}>
          <div style={{ width: 32, height: 32, borderRadius: 9, background: 'linear-gradient(135deg, #57997f 0%, #34685a 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(45,93,232,.3)' }}>
        ...
        <div style={{ background: '#fff', border: `1px solid ${LINE}`, borderRadius: 16, boxShadow: '0 8px 30px rgba(15,21,32,.06)', padding: '28px 26px' }}>
          ...
          {!CONFIGURED ? (
            <div style={{ fontSize: 13, color: '#92400e', background: '#f6efdd', border: '1px solid #f6e0b5', borderRadius: 10, padding: '12px 14px', lineHeight: 1.5 }}>
              Authentication isn&apos;t configured for this deployment. This is the synthetic demo —
              explore it at <Link href="/dashboard" style={{ color: '#3f7d6a', fontWeight: 600 }}>/dashboard</Link>. Set the Supabase
            ...
            <form onSubmit={onSubmit}>
              ...
              {error && <p style={{ fontSize: 12.5, color: '#cf5547', margin: '4px 0 0' }}>{error}</p>}
              {notice && <p style={{ fontSize: 12.5, color: '#2f8a5b', margin: '4px 0 0' }}>{notice}</p>}

              <button type="submit" disabled={loading} style={{ width: '100%', marginTop: 16, fontSize: 14, fontWeight: 600, color: '#fff', background: 'linear-gradient(135deg, #57997f 0%, #34685a 100%)', border: 'none', borderRadius: 10, padding: '11px 0', cursor: loading ? 'default' : 'pointer', opacity: loading ? 0.65 : 1, boxShadow: '0 4px 14px rgba(45,93,232,.22)' }}>
                {loading ? 'Please wait…' : mode === 'signin' ? 'Sign in' : 'Create account'}
              </button>

              <p style={{ fontSize: 12.5, color: SUB, textAlign: 'center', margin: '16px 0 0' }}>
                {mode === 'signin' ? "Don't have an account? " : 'Already have an account? '}
                <button type="button" onClick={() => { setMode(mode === 'signin' ? 'signup' : 'signin'); setError(null); setNotice(null) }} style={{ background: 'none', border: 'none', color: '#3f7d6a', fontWeight: 600, fontSize: 12.5, cursor: 'pointer', padding: 0 }}>
        ...
        <p style={{ fontSize: 11.5, color: '#9aa69f', textAlign: 'center', marginTop: 16, lineHeight: 1.5 }}>
