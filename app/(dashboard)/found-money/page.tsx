  ...
  if (!report) {
    return (
      <div style={{ padding: '28px 32px', maxWidth: '1100px', margin: '0 auto' }}>
        <h1 style={{ fontSize: '20px', fontWeight: 600, color: '#1f2d27', margin: '0 0 4px', letterSpacing: '-0.02em' }}>
          Found Money
        </h1>
    ...
    <div style={{ padding: '28px 32px', maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <p style={{ fontSize: '11px', fontWeight: 600, color: '#9aa69f', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 2px' }}>
            Total recoverable
          </p>
          <p style={{ fontSize: '30px', fontWeight: 700, color: '#2f8a5b', margin: 0, letterSpacing: '-0.02em' }}>
            {formatCents(totals.recoverableCents)}
          </p>
          <a href="/api/found-money/export?format=csv" style={{ display: 'inline-block', marginTop: 8, fontSize: 12, fontWeight: 600, color: '#3f7d6a', textDecoration: 'none', border: '1px solid #d6e2da', background: '#e7f0eb', borderRadius: 8, padding: '5px 11px' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #ece7dd', borderRadius: 12, boxShadow: '0 1px 3px rgba(15,21,32,0.04)', overflow: 'hidden' }}>
        <div style={{ padding: '13px 16px', borderBottom: '1px solid #f0ece3', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: '13px', fontWeight: 600, color: '#1f2d27' }}>Ranked findings</span>
          <span style={{ fontSize: '12px', color: '#9aa69f' }}>by recoverable dollars</span>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {findings.map((finding) => (
              <tr key={finding.id}>
                <td style={{ ...td, textAlign: 'right', fontWeight: 600, color: finding.recoverableCents > 0 ? '#2f8a5b' : '#9aa69f', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>
                ...
                <td style={{ ...td, color: '#5a6b62', maxWidth: 360 }}>{finding.reason}</td>
                <td style={td}>
      ...
      <p style={{ fontSize: '12px', color: '#9aa69f', marginTop: 14, lineHeight: 1.5 }}>
