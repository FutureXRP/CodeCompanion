...
const STATUS_STYLE: Record<string, { fg: string; bg: string; label: string }> = {
  active: { fg: '#2f8a5b', bg: '#e6f4ec', label: 'Active coverage' },
  inactive: { fg: '#cf5547', bg: '#fae9e6', label: 'Inactive / terminated' },
  unknown: { fg: '#92400e', bg: '#f6efdd', label: 'Coverage unknown' },
}
...
export function EligibilityPanel({ configured, sandbox }: { configured: boolean; sandbox: boolean }) {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: '#9aa69f', marginTop: 16, lineHeight: 1.5 }}>
        {configured
          ? sandbox
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${LINE}`, borderRadius: 12, overflow: 'hidden' }}>
      ...
      {result.errors && result.errors.length > 0 && (
        <div style={{ padding: '10px 18px', fontSize: 12.5, color: '#cf5547', borderBottom: `1px solid ${LINE}` }}>
      ...
      <details style={{ borderTop: `1px solid ${LINE}` }}>
        ...
        <pre style={{ margin: 0, padding: '12px 18px', fontFamily: 'DM Mono, ui-monospace, monospace', fontSize: 11.5, lineHeight: 1.5, color: '#3a4640', whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: 360, overflow: 'auto', background: '#faf7f1' }}>
...
function BenefitsTable({ benefits }: { benefits: BenefitItem[] }) {
  ...
  const td: React.CSSProperties = { fontSize: 12.5, color: INK, padding: '8px 12px', borderBottom: `1px solid #f0ece3`, verticalAlign: 'top' }
  return (
          ...
          {benefits.map((b, i) => (
            <tr key={i}>
              <td style={td}>
                ...
                <span style={{ color: '#9aa69f', marginLeft: 6, fontSize: 11 }}>{safeStr(b.code)}</span>
              </td>
